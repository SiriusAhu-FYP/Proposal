...
另外，也有从视频侧直接推断 PX 的做法，为交互或评测中的“机会对齐”提供情绪/投入度的代理\cite{letsplay-px}.
...
更广泛的综述从方法、应用与挑战三方面回顾了 \emph{LLM×游戏} 的研究脉络，涵盖从零样本/检索增强到
模仿/强化学习及\emph{planning/memory/reflection} 与工具调用（tool-use/MCP-style）的组合，并串联环境类型、
输入模态与动作接口以对齐评测与基准的讨论\cite{game-agents-large-models}.
...
该综述亦梳理了多样基准与评测设置，提示跨游戏可比性与协议一致的重要性\cite{game-agents-large-models}.
...
的代理信号：以画面、解说音频等多模态特征弱/自监督提炼 \emph{arousal/valence/engagement}
并与小规模人工评分/高激励事件对齐，用于机会窗口（opportunity windows）与提示时机（hint timing）的度量\cite{letsplay-px}.
...
部署层面，on-device LMs 的综合回顾强调\emph{量化（INT4/FP8）}、\emph{蒸馏/适配}、\emph{KV 缓存与分页注意力}
以及\emph{speculative/streaming 解码}等路径以降低时延与能耗，并讨论端侧资源约束下的稳定性折中与隐私优势\cite{ondevice-llm}.
