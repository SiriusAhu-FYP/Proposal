...
* **OAS (Opportunity-Normalized Success)**: Measures the AI's ability to act on available opportunities, normalized for task complexity. This helps evaluate how well the AI identifies and capitalizes on opportunities in complex, dynamic environments (ORAK, 2023).
* **Dense and Auxiliary Rewards**: A combination of **dense rewards** (feedback at each timestep) and **auxiliary rewards** (additional task-specific rewards, such as **correct formatting** or achieving **sub-goals**) accelerates learning and improves task performance, particularly in complex tasks (V-Mage, 2024).
...
* **Skill Curation**:
  A **modular approach** to task-solving that involves breaking down actions into **atomic skills** that can be recombined dynamically. **UI-Venus (2023)** introduces the concept of **modular skill sets** for task-solving, which allows for better handling of long-term, dynamic tasks. This approach supports the agent's adaptability by enabling it to decompose complex tasks into manageable sub-goals (UI-Venus, 2023).
...
* **Out‑of‑Distribution (OOD) Generalization**:
  Both **V-Mage (2024)** and **Benchmarking-VLA-VLM (2025)** identify OOD generalization as a critical issue for AI companions. In particular, vision‑based models face difficulties in adapting to new, unseen environments, while structured outputs (e.g., text‑oriented) tend to generalize more reliably. This challenge needs to be addressed for developing robust, adaptable AI companions that can perform consistently across a wide range of games and environments.
* **Future Directions**:
